 ...
 */
 ...
 class Camera {
  ...
  void orient() {
    ...
    // Camera Position
    eyeX = boundary.x * 0.5;
    //eyeY = (zoom/ZOOM_MIN - 0.5) * boundary.y;
  ...
  void reset() {
    ...
    vs.newspos = vs.sposMin + abs(ZOOM_MIN-ZOOM_DEFAULT)/abs(ZOOM_MIN-ZOOM_MAX)*vs.sheight;
    drag.x_offset = 0;
    drag.y_offset = 0;
  ...
  class VScrollbar {
    ...
    float getPosZoom() {
      // Convert spos to be values between
      // 0 and 2PI
      return ZOOM_MIN + (ZOOM_MAX - ZOOM_MIN) * spos / float(sheight);
    }
  }
